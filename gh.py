...
class GHApi:
    ...
    def upload_code(self, readme_content: str, title_slug: str, title: str, question_frontend_id: str, runtime: str,
                    runtime_faster_than: float, memory: str, memory_less_than: float, code: str, lang_name: str):
        folder = f"{question_frontend_id}-{title_slug}"
        file = f"{question_frontend_id}. {title}.{self.lang_map.get(lang_name, lang_name)}"
        readme = "README.md"
        readme_path = f"{self.host}/repos/{self.owner}/{self.repo}/contents/src/{folder}/{readme}"
        upsert_readme_result = self.upsert(readme_path, "README", readme_content)
        debug("upsert readme result", upsert_readme_result.text)
        code_path = f"{self.host}/repos/{self.owner}/{self.repo}/contents/src/{folder}/{file}"
        upsert_code_result = self.upsert(code_path,
                                         f"Runtime: {runtime} {f'(faster than {runtime_faster_than}%)' if runtime_faster_than > 0 else ''}, Memory: {memory} {f'(less than {memory_less_than}%)' if memory_less_than > 0 else ''} - GitSync",
                                         code)
        debug("upsert code result", upsert_code_result.text)
        info(
            f"failed to upload question {question_frontend_id}") if upsert_code_result.status_code // 100 != 2 else info(
            f"successfully uploaded quesetion {question_frontend_id}")
